/* ============================================================
   STÉPHANE MAGRE — i18n
   Default: French (fr). Toggle to English (en).
   ============================================================ */

const T = {
  fr: {
    'nav.home':'Accueil','nav.projects':'Projets','nav.about':'À propos','nav.contact':'Contact',
    'lang.other':'EN',
    'footer.sub':'Paysagiste · Paris','footer.copy':'© 2026 Stéphane Magre',
    'btn.view-projects':'Voir les projets','btn.all-projects':'Tous les projets',
    'btn.contact':'Contact','btn.approach':'L\'approche','btn.in-touch':'Prendre contact',
    'btn.book':'Réserver un appel',
    'btn.etude':'Demander une étude de parc',
    'btn.back-projects':'Tous les projets','btn.next-project':'Projet suivant','btn.first-project':'Premier projet',

    /* Homepage */
    'home.hero.eyebrow':'Paris · Toute la France',
    'home.hero.name':'Stéphane Magre',
    'home.hero.role':'Paysagiste',
    'home.hero.lines':'Parcs, jardins et paysages<br>Châteaux – domaines historiques – propriétés d\'exception<br>Paris – France – International',
    'home.hero.title':'Des parcs et jardins<br>à la hauteur<br>de votre demeure.',
    'home.hero.sub':'Conception et pilotage de projets de paysage —<br>châteaux, domaines historiques et propriétés d\'exception.',
    'home.statement.label':'La pratique',
    'home.statement.h2':'Là où le patrimoine<br>rencontre le paysage vivant.',
    'home.statement.p1':'Chaque projet naît d\'une lecture fine du site — sa topographie, sa géographie, son patrimoine végétal. Cette attention portée au vivant et aux lieux construit des espaces cohérents, adaptés à leurs usages et à leur environnement.',
    'home.statement.p2':'De la résidence privée à la résidence de l\'Ambassadeur de France, j\'assume la pleine cohérence du résultat — un accompagnement complet, de l\'esquisse à la livraison, et au-delà.',
    'home.projects.label':'Projets sélectionnés',
    'home.projects.h2':'Une sélection soignée<br>de commandes.',
    'home.approach.label':'Ma démarche',
    'home.approach.h2':'L\'écoute, la collaboration<br>et le vivant.',
    'home.approach.01.h4':'Les commanditaires',
    'home.approach.01.p':'La relation de confiance avec les clients est essentielle : elle permet de construire un projet réellement partagé, fondé sur leurs attentes et enrichi par mon expertise — de la conception au suivi de chantier, dans un dialogue constant.',
    'home.approach.02.h4':'La collaboration',
    'home.approach.02.p':'Je collabore avec les architectes, les décorateurs et les urbanistes pour créer des espaces extérieurs singuliers. Le travail en commun constitue un apport créatif et intellectuel qui nourrit les projets.',
    'home.approach.03.h4':'Le végétal comme élément de projet',
    'home.approach.03.p':'Le végétal constitue la matière première du projet de paysage. Par son architecture, il structure l\'espace et lui donne sens — un levier puissant pour façonner des projets cohérents et sensibles.',
    'home.expertise.label':'Domaines d\'intervention',
    'home.expertise.h2':'Une expertise du paysage,<br>du château à la côte.',
    'home.expertise.01.h4':'Parcs de châteaux & domaines historiques',
    'home.expertise.01.p':'Restauration, replantation et réinterprétation de parcs de châteaux, domaines historiques et demeures de caractère. Structures végétales, alignements, bosquets et grandes compositions paysagères sont repensés dans un regard contemporain, entre continuité historique et transformation du lieu.',
    'home.expertise.02.h4':'Jardins privés',
    'home.expertise.02.p':'Villas, résidences d\'exception et maisons de famille. Des jardins conçus à partir du lieu et de ses usages, où les espaces, les plantations et les vues composent un paysage à vivre.',
    'home.expertise.03.h4':'Jardins méditerranéens & littoraux',
    'home.expertise.03.p':'Une pratique attentive aux spécificités des milieux méditerranéens et littoraux. Sols, végétation, lumière, climat et relation au paysage fondent la conception du jardin. Des palettes végétales sobres en eau, adaptées aux conditions du milieu et évolutives.',
    'home.expertise.04.h4':'Hôtels et lieux d\'accueil',
    'home.expertise.04.p':'Des paysages conçus pour accompagner l\'architecture et les différents usages du lieu. Jardins, patios, terrasses, piscines et toitures végétalisées sont pensés avec une attention particulière portée aux parcours, aux vues et à l\'intimité des espaces.',
    'home.expertise.05.h4':'Conception et suivi de réalisation',
    'home.expertise.05.p':'De l\'esquisse au suivi de chantier, j\'assure la conception du projet, l\'élaboration des dossiers techniques et le suivi de sa réalisation, en dialogue avec les différents intervenants.',
    'home.expertise.06.h4':'Études paysagères',
    'home.expertise.06.p':'Diagnostic paysager, lecture historique et analyse de site pour comprendre ses structures, ses qualités et son potentiel. Une vision d\'ensemble pour orienter et accompagner les transformations d\'un site.',

    'home.heritage.label':'Demeures & parcs historiques',
    'home.heritage.h2':'Votre parc a une histoire.<br>Donnons-lui un avenir.',
    'home.heritage.p1':'Le parc d\'une demeure historique n\'est pas un jardin comme les autres. C\'est un héritage vivant — tracés d\'origine, perspectives, arbres centenaires, murs et bassins oubliés — qui demande une lecture patiente avant tout dessin.',
    'home.heritage.p2':'Le changement climatique met aujourd\'hui ce patrimoine à l\'épreuve : buis, hêtres et sujets centenaires souffrent partout en France. Je conçois des plans de restauration et de replantation qui respectent l\'esprit du lieu tout en le préparant aux cinquante prochaines années.',
    'home.heritage.offer.strong':'L\'Étude de parc.',
    'home.heritage.offer.p':'Une visite approfondie, une lecture historique et paysagère du site, un concept directeur et une feuille de route chiffrée — le point de départ de toute transformation sérieuse.',
    'home.credibility.label':'Expérience',
    'home.credibility.h2':'La confiance de clients<br>exigeants et discrets.',
    'home.credibility.p1':'De la résidence de l\'Ambassadeur de France à La Havane aux hôtels de prestige réalisés avec Bouygues Bâtiment International, en passant par des domaines privés à travers la France — chaque commande exige le même niveau de soin, de discrétion et de précision.',
    'home.credibility.p2':'Je collabore régulièrement avec des architectes, des architectes du patrimoine, des décorateurs et des agences immobilières de prestige qui recherchent un partenaire paysagiste capable d\'intervenir au plus haut niveau.',
    'home.credibility.stat1.label':'Années de pratique',
    'home.credibility.stat2.label':'Commandes réalisées',
    'home.credibility.stat3.label':'Continents',
    'home.credibility.stat4.label':'Plus grand domaine',
    'home.cta.label':'Commandes',
    'home.cta.h2':'Pour des commandes<br>et collaborations privées.',
    'home.cta.p':'Les projets sont acceptés de façon sélective. Si votre demeure — château, domaine ou propriété d\'exception — mérite une vision paysagère sérieuse, prenez contact.',

    /* Projects page */
    'projects.hero.label':'Réalisations','projects.hero.h1':'Projets',
    'projects.hero.p':'Une sélection de projets réalisés en France et à l\'international.',
    'projects.cta.label':'Commandes',
    'projects.cta.h2':'Pour une commande<br>ou une collaboration.',
    'projects.cta.p':'Les commandes sont acceptées de façon sélective. N\'hésitez pas à me contacter pour discuter de votre projet.',

    /* About page */
    'about.hero.label':'La pratique','about.hero.h1':'À propos',
    'about.hero.p':'Stéphane Magre est paysagiste, diplômé de l\'École Nationale Supérieure du Paysage de Versailles.',
    'about.hero.p2':'Il conçoit et assure le suivi de jardins et de paysages sur mesure, en France et à l\'international — de la conception à la livraison.',
    'about.intro.label':'Philosophie',
    'about.intro.lead':'Mon travail repose sur une lecture fine du site — sa topographie, sa géographie, son patrimoine végétal. Chaque projet naît de cette attention portée au lieu et au vivant.',
    'about.intro.p1':'Cette démarche construit des paysages cohérents, adaptés à leurs usages et à leur environnement. La composition s\'appuie sur une palette végétale précise, pensée comme une matière évolutive et durable, capable de répondre aux transformations du climat. La forme du projet découle ainsi des qualités du site, de ses usages et des dynamiques du vivant.',
    'about.intro.p2':'L\'enjeu est de concevoir des paysages dynamiques, ancrés dans leur contexte, respectueux des équilibres écologiques et ouverts à l\'évolution du vivant — des paysages supports de biodiversité, où la forme et la fonction s\'accordent avec les processus du vivant.',
    'about.philosophy.label':'Principes','about.philosophy.h2':'Comment j\'aborde<br>le projet de paysage.',
    'about.philo.01.h4':'La lecture du site',
    'about.philo.01.p':'Chaque site a une logique — une topographie, une géographie, un patrimoine végétal, une écologie existante. Le projet naît de l\'observation et de la compréhension de ces différentes strates. La forme du projet découle de cette compréhension : regarder avant de dessiner, comprendre avant de transformer.',
    'about.philo.02.h4':'Le végétal comme matière première',
    'about.philo.02.p':'Le végétal constitue la matière première du projet de paysage. Par sa structure, ses volumes, ses textures et ses saisons, il organise l\'espace, compose les vues et accompagne les usages. En dialogue avec les qualités propres du site, il permet de façonner des paysages cohérents, sensibles et vivants.',
    'about.philo.03.h4':'Une palette végétale pour le siècle à venir',
    'about.philo.03.p':'Le climat transforme les conditions de nos paysages. Je compose des palettes végétales capables d\'évoluer avec elles, en conciliant résilience, pérennité et identité du lieu. Préserver l\'esprit d\'un paysage, c\'est aussi anticiper ce qu\'il deviendra demain.',
    'about.philo.04.h4':'Des paysages pour le vivant',
    'about.philo.04.p':'Je compose des paysages dynamiques, ancrés dans leur contexte, qui accueillent les différentes formes du vivant et s\'inscrivent dans les équilibres écologiques du lieu. Leur pérennité repose sur leur capacité à évoluer avec le vivant et à s\'adapter aux transformations du milieu.',
    'about.philo.05.h4':'En dialogue avec l\'architecture',
    'about.philo.05.p':'Le paysage se construit aussi dans son dialogue avec l\'architecture. Je porte une attention particulière aux proportions, aux matières, aux cadrages et aux usages, qui donnent au projet sa cohérence et sa profondeur. Le paysage n\'est alors ni un décor ni un espace résiduel, mais une composante à part entière du projet.',
    'about.philo.06.h4':'Concevoir, réaliser, accompagner',
    'about.philo.06.p':'J\'accompagne chaque projet de la conception au suivi de chantier, dans un dialogue constant avec les différents intervenants. Une fois le projet réalisé, la relation peut se poursuivre autour de son entretien, de son évolution et de sa pérennité dans le temps.',
    'about.role.label.1':'Les commanditaires','about.role.h2.1':'Un projet<br>réellement partagé.',
    'about.role.p1.1':'La relation de confiance avec les commanditaires est au cœur de chaque projet. Elle permet d\'écouter leurs attentes, de comprendre leurs usages et de faire émerger un projet qui leur ressemble, enrichi par mon regard et mon expertise. Le dialogue se poursuit tout au long de la conception et de la réalisation, pour construire ensemble un projet juste, cohérent et durable. Une fois le jardin réalisé, je peux également accompagner son évolution dans le temps, afin d\'en préserver la qualité et l\'esprit.',
    'about.role.p2.1':'Une fois le jardin réalisé, la relation se poursuit grâce à un accompagnement sur l\'entretien et l\'évolution du projet, pour en assurer la pérennité et la qualité dans le temps.',
    'about.role.label.2':'La collaboration','about.role.h2.2':'Architectes, décorateurs<br>et urbanistes.',
    'about.role.p1.2':'Je travaille aux côtés d\'architectes, de décorateurs, d\'urbanistes, d\'écologues et autres professionnels du projet. La complémentarité des regards et des savoir-faire nourrit la réflexion et enrichit les projets. Selon les contextes, j\'interviens au sein d\'équipes constituées ou directement auprès des commanditaires, en apportant mon regard de paysagiste et en veillant à la cohérence de l\'ensemble.',
    'about.role.p2.2':'Ces collaborations permettent d\'intervenir au même niveau que les agences avec lesquelles je travaille — sur des résidences privées comme sur des projets d\'envergure internationale.',
    'about.portrait.label':'Parcours','about.portrait.h2':'Du diplôme<br>à la pratique.',
    'about.portrait.p1':'Diplômé Paysagiste DPLG de l\'École Nationale Supérieure du Paysage de Versailles — avec un projet de diplôme consacré au parc de Saint-Cloud — j\'ai complété ma formation par des expériences à l\'étranger qui ont largement influencé ma façon de penser le paysage.',
    'about.portrait.p2':'Après une dizaine d\'années consacrées à des projets internationaux d\'envergure — ambassades, hôtels, domaines — j\'ai choisi de consacrer ma pratique aux projets privés, aux parcs, aux demeures historiques et aux propriétés d\'exception. Une pratique fondée sur une attention particulière au lieu, au végétal et au temps long.',
    'about.portrait.p3':'Basé à Paris, j\'interviens partout en France et à l\'international.',

    /* Contact page */
    'contact.hero.label':'Prendre contact','contact.hero.h1':'Contact',
    'contact.hero.p':'J\'interviens sur des projets privés — châteaux, domaines, propriétés d\'exception — ainsi que dans le cadre de collaborations avec des architectes et des décorateurs ou pour des demandes d\'études.',
    'contact.hero.p2':'Chaque projet commence par un échange, afin de comprendre son histoire, ses enjeux et les attentes de ses commanditaires. Je privilégie les projets dont le contexte, l\'ambition et la démarche font écho à ma pratique et à mon regard de paysagiste.',
    'contact.info.label':'Un échange direct',
    'contact.info.h2':'Premier échange.',
    'contact.info.p1':'La façon la plus directe de me joindre est par email. Je réponds personnellement à chaque demande, généralement dans les deux jours ouvrés.',
    'contact.info.p2':'Si vous préférez échanger de vive voix, envoyez-moi un court message par email. Je vous proposerai alors un appel à un moment qui vous convient.',
    'contact.link.email.label':'E-mail','contact.link.phone.label':'Téléphone',
    'contact.link.linkedin.label':'LinkedIn','contact.link.location.label':'Localisation',
    'contact.link.location.value':'Paris, France',
    'contact.note.p':'Les projets sont acceptés de façon sélective. J\'interviens sur un nombre limité de commandes à la fois, et consacre à chacune toute mon attention. Si je ne suis pas disponible pour votre calendrier, je vous le dirai directement.',
    'contact.faq.label':'Informations','contact.faq.h2':'Questions fréquentes.',
    'contact.faq.q1':'Où intervenez-vous ?',
    'contact.faq.a1':'Basé à Paris, j\'interviens dans toute la France — Île-de-France, Provence, Côte d\'Azur, Auvergne, littoral atlantique — ainsi qu\'à l\'international.',
    'contact.faq.q2':'Travaillez-vous sur les parcs de châteaux et demeures historiques ?',
    'contact.faq.a2':'Oui. La restauration et la replantation de parcs de châteaux, de monuments et de propriétés d\'exception sont au cœur de ma pratique.',
    'contact.faq.q3':'Sur quels types de projets intervenez-vous ?',
    'contact.faq.a3':'Jardins de propriétés privées, parcs de demeures historiques, jardins méditerranéens et littoraux, hôtels et lieux d\'accueil — en France comme à l\'international.',
    'contact.faq.q4':'Assurez-vous le suivi des travaux ?',
    'contact.faq.a4':'Oui — un accompagnement complet : conception, dossiers d\'exécution et suivi de chantier, jusqu\'à la livraison.',
    'contact.faq.q5':'Comment démarrer un projet ?',
    'contact.faq.a5':'Par un simple message ou un appel. Pour les parcs, je propose une étude de parc : diagnostic, concept directeur et feuille de route chiffrée.',
    'contact.faq.q0':'Qui est Stéphane Magre ?',
    'contact.faq.a0':'Stéphane Magre est paysagiste, diplômé de l\'École Nationale Supérieure du Paysage de Versailles. Basé à Paris, il conçoit et assure le suivi de parcs, de jardins et de paysages sur mesure — châteaux, domaines historiques, propriétés d\'exception — en France et à l\'international.',
    'contact.faq.q6':'Pouvez-vous intervenir sur un jardin existant ?',
    'contact.faq.a6':'Oui. À partir d\'un diagnostic du site, je peux restructurer, replanter ou faire évoluer un jardin existant, sans nécessairement tout reprendre.',

    /* Shared project labels */
    'project.eyebrow':'Commande',
    'project.meta.location':'Localisation','project.meta.scope':'Mission',
    'project.meta.type':'Type','project.meta.status':'Statut',
    'project.meta.partner':'En collaboration avec',
    'project.desc.label':'Intention du projet',
    'project.nav.back':'Tous les projets','project.nav.next.label':'Projet suivant',

    /* Villa Privée */
    'villa.h1':'Villa Privée','villa.meta.location':'France',
    'villa.meta.scope':'ESQ, PRO, DCE, EXE','villa.meta.type':'Résidence privée, 8 000 m²',
    'villa.meta.status':'Réalisé',
    'villa.desc.h3':'Une villa en continuité<br>avec le paysage.',
    'villa.desc.p1':'Dès l\'entrée, la villa s\'affirme par une architecture contemporaine, définie par des lignes épurées et un porche en porte-à-faux qui crée une sensation à la fois surprenante et aérienne. Le jardin d\'accueil prolonge ce geste architectural grâce à une composition ornementale où les essences sont choisies pour leur valeur décorative, soulignant ainsi le caractère affirmé du lieu.',
    'villa.desc.p2':'En parcourant les espaces intérieurs, la perception s\'inverse. Côté mer, la villa s\'efface pour laisser toute la place au site. Les volumes se fondent dans la topographie et s\'ancrent dans le paysage. Des terrasses discrètes s\'insèrent dans le terrain naturel, ouvrant des vues cadrées vers l\'horizon et les éléments.',
    'villa.desc.p3':'Ici, le jardin n\'est plus un décor ajouté, mais la continuité même du paysage. Il est imaginé à partir de végétaux autochtones et endémiques, déjà présents sur le site : collectés avec soin, multipliés en pépinière, puis réintroduits pour recréer un écosystème vivant et cohérent.',
    'villa.nav.prev':'Tous les projets','villa.nav.next.title':'Ambassade de France à Cuba',

    /* Ambassade */
    'ambassade.h1':'Ambassade de France à Cuba','ambassade.meta.location':'La Havane, Cuba',
    'ambassade.meta.scope':'Jardins, espace culturel','ambassade.meta.type':'Résidence diplomatique, 8 000 m²',
    'ambassade.meta.status':'Réalisé',
    'ambassade.desc.h3':'Un jardin de mémoire<br>et de rassemblement.',
    'ambassade.desc.p1':'Le jardin de la résidence de France a été pensé comme un lieu de partage et de mémoire, un espace qui raconte l\'histoire du lieu tout en l\'ancrant dans le présent.',
    'ambassade.desc.p2':'Le projet s\'appuie sur les matériaux et les traces existantes : pierres anciennes, bassins oubliés et végétation en place. Grands ficus, palmiers royaux et massifs denses de végétaux exotiques, autant de présences respectées, retravaillées et ouvertes à la lumière pour redonner souffle et équilibre au jardin.',
    'ambassade.desc.p3':'Au cœur de la résidence, une grande pelouse s\'ouvre en perspective vers la statue, mettant en relation la villa et la profondeur de ce paysage miniature. Autour de cet axe se déploient des espaces plus intimes : jardins cachés, piscine, chambres de verdure et potager. Entre héritage et vitalité, le jardin retrouve son rôle : accueillir, rassembler et réinscrire le site dans une dynamique vivante et écologique.',
    'ambassade.nav.next.title':'Jardin Privé, Auvergne',

    /* Jardin Privé */
    'jardin.h1':'Jardin Privé','jardin.meta.location':'Auvergne, France',
    'jardin.meta.scope':'Conception complète &amp; exécution','jardin.meta.type':'Propriété privée, 2 ha',
    'jardin.meta.status':'Réalisé',
    'jardin.desc.h3':'Un lieu habité,<br>ouvert à son environnement.',
    'jardin.desc.p1':'Ce jardin privé, situé en Auvergne, s\'étend sur deux hectares. Il s\'articule autour d\'une grande prairie fauchée une fois l\'an, qui s\'ouvre sur un vallon doux où coule un ruisseau. En contrebas, un ancien lavoir a été redécouvert, ainsi qu\'une minuscule pièce d\'eau nichée entre de grands saules blancs, point de fraîcheur discret qui prolonge la perspective depuis la terrasse de la maison.',
    'jardin.desc.p2':'De là, la vue offre une lecture claire sur le vallon et les reliefs environnants. Autour de l\'ancien corps de ferme, le jardin se déploie en une série d\'espaces de vie : jardin d\'ombre, potager, jardin d\'ornement et verger replanté.',
    'jardin.desc.p3':'L\'ensemble compose un lieu à la fois habité et ouvert sur son environnement, un écosystème qui accompagne les dynamiques naturelles du site, profondément écologique.',
    'jardin.nav.next.title':'Hôtel Iberostar Grand Packard',

    /* Iberostar */
    'iberostar.h1':'Hôtel Iberostar Grand Packard','iberostar.meta.location':'La Havane, Cuba',
    'iberostar.meta.scope':'Espaces extérieurs, patio, terrasses, piscine','iberostar.meta.type':'Hôtel de luxe, 3 000 m²',
    'iberostar.meta.status':'Réalisé',
    'iberostar.meta.partner':'Bouygues Bâtiment International',
    'iberostar.desc.h3':'Des jardins suspendus<br>au-dessus de La Havane.',
    'iberostar.desc.p1':'À l\'entrée du Paseo del Prado, l\'hôtel Grand Packard de La Havane est un établissement 4 étoiles qui se distingue autant par son architecture élégante que par la richesse de ses jardins. Conçu comme un véritable écrin de verdure, il déploie des espaces végétalisés à différents niveaux, offrant aux visiteurs une expérience sensorielle unique au cœur de la ville.',
    'iberostar.desc.p2':'Au rez-de-chaussée, le patio intérieur s\'inspire des grandes demeures de La Havane, où l\'on retrouve ces cours ombragées, ponctuées d\'eau et conçues comme des refuges rafraîchissants au cœur de la chaleur urbaine. Directement relié au bar et aux espaces festifs de l\'hôtel, ce patio devient un lieu de convivialité animé, accueillant concerts et évènements. Cet espace minéral, habillé de grandes dalles d\'ardoise, s\'ouvre sur un bassin et une composition végétale simple : des palmiers Licuala grandis créant une ombre légère et mouvante, tandis qu\'en sous-étage fougères et liriopes tapissent le sol d\'un tapis végétal luxuriant.',
    'iberostar.desc.p3':'À l\'opposé, les terrasses situées au dernier étage offrent un tout autre univers. Elles déploient de véritables jardins suspendus, reposant sur 60 cm de terre. En strates hautes, les élégants Cocothrinax crinita structurent l\'espace, tandis qu\'autour prospère une profusion de plantes exubérantes : Heliconia, Alpinia, Calathea, Dieffenbachia, Monstera, Dracaena et Alocasia. Entre l\'élégance minérale du patio et la luxuriance des terrasses, l\'hôtel offre deux atmosphères complémentaires : l\'une propice aux rencontres et aux festivités, l\'autre intime et contemplative, ouverte sur des horizons de verdure.',
    'iberostar.nav.next.title':'Les Normaliennes',

    /* Les Normaliennes */
    'normaliennes.h1':'Les Normaliennes','normaliennes.meta.location':'Auvergne, France',
    'normaliennes.meta.scope':'Mandataire — APD, PRO, DCE, EXE','normaliennes.meta.type':'Jardin urbain, 2 ha',
    'normaliennes.meta.status':'Réalisé',
    'normaliennes.meta.partner':'Cap Paysages',
    'normaliennes.desc.h3':'Un jardin traversant<br>au cœur de l\'îlot.',
    'normaliennes.desc.p1':'Le projet de requalification des espaces extérieurs de l\'ancienne IUFM redonne à cet îlot urbain une véritable cohérence paysagère et une valeur d\'usage renouvelée. Au cœur d\'un ensemble mêlant école et logements, le site fait l\'objet d\'une transformation majeure : créer une allée piétonne traversante tout en installant un jardin généreux au cœur de l\'îlot.',
    'normaliennes.desc.p2':'L\'allée traversante, réalisée en pavés de pierre de Volvic et déclinée en dallages, stabilisé minéral, banquettes et grandes dalles de forme naturelle, structure le site et lui donne une identité commune.',
    'normaliennes.desc.p3':'De part et d\'autre, un jardin linéaire de près de deux hectares se déploie, composé majoritairement de graminées auxquelles s\'ajoutent vivaces, euphorbes, pétasites et sedums, ponctué de plantations structurantes de magnolias et d\'amélanchiers. L\'ensemble forme un jardin à la fois libre dans son expression végétale et rigoureusement dessiné, offrant une continuité paysagère de façade à façade.',
    'normaliennes.nav.next.title':'Hôtel Melina International',

    /* Hôtel Melina International */
    'melina.h1':'Hôtel Melina International','melina.meta.location':'Varadero, Cuba',
    'melina.meta.scope':'ESQ, APD, PRO, DCE, EXE','melina.meta.type':'Hôtel de luxe, 6 ha',
    'melina.meta.status':'Réalisé',
    'melina.meta.partner':'Bouygues Bâtiment International',
    'melina.desc.h3':'Un jardin botanique<br>face à la mer.',
    'melina.desc.p1':'Le jardin de l\'Hôtel International s\'étend sur six hectares, enveloppant un bâtiment imposant dans un foisonnement végétal. Conçu comme un véritable jardin botanique, il offre aux visiteurs une immersion dans la diversité tropicale, révélant l\'identité du site à travers la richesse de ses essences. Les tracés organiques et les chemins en creux invitent à une promenade au cœur d\'une végétation stratifiée, où couvre-sols, arbustes, palmiers et grands arbres s\'entrelacent.',
    'melina.desc.p2':'Une large partie du jardin se déploie entre le bâtiment et la mer, où la piscine se fond dans ce paysage tropical luxuriant. On y découvre des espèces emblématiques telles que les héliconias, dracénas, alocasias, alpinias, monsteras, aralias, cannes à sucre, cycas revoluta et frangipaniers, complétées de lyriopes et de spathiphyllums en couvre-sol.',
    'melina.desc.p3':'Palmiers royaux, Washingtonia robusta, cocotiers, Beaucarnea recurvata et palmiers de Santa Lucia se mêlent aux Schefflera, Cordia sebestena et Plumeria, composant une canopée dense et une profusion végétale tropicale en bord de mer.',
    'melina.nav.next.title':'Hôtel Prado Malecón',

    /* Hôtel Prado Malecón */
    'prado.h1':'Hôtel Prado Malecón','prado.meta.location':'La Havane, Cuba',
    'prado.meta.scope':'APD, PRO, DCE, EXE','prado.meta.type':'Hôtel de luxe, 1 000 m²',
    'prado.meta.status':'Réalisé',
    'prado.meta.partner':'Bouygues Bâtiment International',
    'prado.desc.h3':'Un cœur végétal<br>entre Prado et Malecón.',
    'prado.desc.p1':'Situé à l\'angle du Paseo del Prado et du Malecón, l\'hôtel du Prado s\'organise autour d\'un patio de 300 m², véritable cœur végétal qui relie les deux ailes du bâtiment. Un cheminement central en terrazzo blanc traverse cet espace de 25 mètres de long, invitant le visiteur à se promener au milieu d\'une végétation tropicale luxuriante.',
    'prado.desc.p2':'Planté en pleine terre, le jardin accueille Dracaena, Heliconia, fougères, Anthurium, Calathea et Begonia, ainsi que des lianes telles que les Monstera deliciosa qui, avec le temps, viendront habiller les câbles en acier inoxydable tendus en hauteur. Des bancs soigneusement disposés offrent des moments de pause dans ce lieu à la fois intime et traversant.',
    'prado.desc.p3':'Les terrasses, exposées au vent marin, sont ponctuées de pots accueillant des palmiers Areca. Depuis les terrasses basses, la mer et la promenade du Malecón s\'étendent devant nous ; depuis les terrasses hautes, le regard embrasse le front de mer et le Paseo del Prado, qui mène vers le centre historique, le Grand Théâtre et le Capitole.',
    'prado.nav.next.title':'Pôle Touristique Cayo Cruz',

    /* Pôle Touristique Cayo Cruz */
    'cayo.h1':'Pôle Touristique Cayo Cruz','cayo.meta.location':'Cayo Cruz, Cuba',
    'cayo.meta.scope':'APD, PRO, DCE, EXE','cayo.meta.type':'Complexe hôtelier, 15 ha',
    'cayo.meta.status':'Réalisé',
    'cayo.meta.partner':'Bouygues Bâtiment International',
    'cayo.desc.h3':'Préserver une île,<br>habiter son paysage.',
    'cayo.desc.p1':'L\'hôtel Coco de Mar est implanté sur une parcelle de 15 hectares, sur une petite île des Caraïbes. Le projet s\'articule autour de bâtiments aux patios circulaires, aux formes organiques, intégrés dans la végétation tropicale existante.',
    'cayo.desc.p2':'L\'enjeu majeur est de préserver le paysage naturel : les circulations et chemins de sable sont redessinés sur site pour contourner les arbres et sujets remarquables, tout en respectant la topographie subtile des lieux, marquée par des sables fossilisés et une flore spécifique.',
    'cayo.desc.p3':'Au cœur de cet environnement, un jardin de loisir accueille une piscine dont les plages aux formes organiques s\'intègrent à la topographie du site. Dans les patios, une palette végétale ornementale dialogue avec les espèces autochtones et endémiques, créant un équilibre entre l\'apport de nouvelles essences et la mise en valeur du patrimoine végétal existant.',
    'cayo.nav.next.title':'Hôtel Corona',

    /* Hôtel Corona */
    'corona.h1':'Hôtel Corona','corona.meta.location':'La Havane, Cuba',
    'corona.meta.scope':'APD, PRO, DCE, EXE','corona.meta.type':'Hôtel de luxe, patio 1 000 m²',
    'corona.meta.status':'Réalisé',
    'corona.meta.partner':'Bouygues Bâtiment International',
    'corona.desc.h3':'Un refuge tropical<br>au centre de la ville.',
    'corona.desc.p1':'Au cœur de cet ancien bâtiment réhabilité en hôtel de cinq étages s\'ouvre un patio central de près de 1 000 m² — écrin minéral et végétal, à la fois lieu de détente et d\'immersion sensorielle. Le sol, en granit noir, capte la lumière et renforce l\'atmosphère feutrée du lieu, entre fraîcheur et profondeur.',
    'corona.desc.p2':'Autour de cet espace s\'épanouit une végétation luxuriante. Pour rejoindre la terrasse centrale, les visiteurs traversent cette épaisseur végétale de deux à cinq mètres sous un couvert de feuillages exotiques, avant de déboucher au cœur du patio, puits de lumière ouvert sur le ciel où sont disposées les tables autour du bar.',
    'corona.desc.p3':'Les plantations composent une riche palette tropicale — Anthurium, Begonia, Calathea et Heliconia en sous-étage ombragé, Dracaena et Monstera deliciosa en trame dense, Licuala grandis, Washingtonia et les majestueux Ravenala madagascariensis, arbres du voyageur, élevant leurs silhouettes au-dessus du patio. Un refuge tropical au centre de la ville.',
    'corona.nav.next.title':'La Traversée',

    /* La Traversée */
    'traversee.h1':'La Traversée','traversee.meta.location':'Côte atlantique, France',
    'traversee.meta.scope':'APD, PRO, DCE, EXE','traversee.meta.type':'Promenade littorale, 800 ml',
    'traversee.meta.status':'Réalisé',
    'traversee.desc.h3':'Une expérience de traversée<br>entre terre et mer.',
    'traversee.desc.p1':'Cette promenade s\'inscrit dans un site naturel — paysage de roches, d\'eau et de végétation littorale halophile.',
    'traversee.desc.p2':'Entre la mer et le village, un espace de transition s\'ouvre, traversé par une rivière qui vient se mêler aux flux marins. C\'est ici que le chemin trouve sa place, non pas comme une rupture, mais comme un dispositif de continuité entre deux entités du site.',
    'traversee.desc.p3':'Plutôt qu\'un axe de liaison, la promenade devient une expérience de traversée.',
    'traversee.nav.next.title':'Villa Privée',

    /* Project card descriptions (projects.html) */
    'card.villa.desc':'Un jardin composé de végétaux autochtones, dans la continuité du paysage et de l\'architecture.',
    'card.ambassade.desc':'Le jardin de la résidence de l\'Ambassadeur de France à La Havane, lieu de partage et de mémoire.',
    'card.jardin.desc':'Une propriété de deux hectares en Auvergne : prairie, lavoir, verger et potager.',
    'card.iberostar.desc':'Un patio au rez-de-chaussée et des jardins suspendus en terrasse, au cœur de La Havane.',
    'card.normaliennes.desc':'La requalification d\'un îlot urbain en Auvergne : une allée en pierre de Volvic et deux hectares de jardins.',
    'card.melina.desc':'Six hectares de jardin botanique tropical en bord de mer, à Varadero.',
    'card.prado.desc':'Un patio-jardin de 300 m² entre le Paseo del Prado et le Malecón.',
    'card.cayo.desc':'Quinze hectares sur une île des Caraïbes, conçus pour préserver le paysage existant.',
    'card.corona.desc':'Un patio de 1 000 m² au cœur d\'un ancien bâtiment réhabilité : granit noir et végétation tropicale.',
    'card.traversee.desc':'Une promenade littorale entre roches, eau et végétation halophile.',
  },

  en: {
    'nav.home':'Home','nav.projects':'Projects','nav.about':'About','nav.contact':'Contact',
    'lang.other':'FR',
    'footer.sub':'Landscape Architect · Paris','footer.copy':'© 2026 Stéphane Magre',
    'btn.view-projects':'View projects','btn.all-projects':'All projects',
    'btn.contact':'Contact','btn.approach':'The approach','btn.in-touch':'Get in touch',
    'btn.book':'Book a call',
    'btn.etude':'Request a park study',
    'btn.back-projects':'All projects','btn.next-project':'Next project','btn.first-project':'Back to first project',

    /* Homepage */
    'home.hero.eyebrow':'Paris · All of France',
    'home.hero.name':'Stéphane Magre',
    'home.hero.role':'Landscape Architect',
    'home.hero.lines':'Parks, gardens and landscapes<br>Châteaux – historic estates – exceptional properties<br>Paris – France – International',
    'home.hero.title':'Parks and gardens<br>worthy of<br>your estate.',
    'home.hero.sub':'Design and full project leadership —<br>châteaux, historic estates and exceptional properties across France.',
    'home.statement.label':'The practice',
    'home.statement.h2':'Where heritage<br>meets the living landscape.',
    'home.statement.p1':'Each project begins with a close reading of the site — its topography, its geography, its living heritage. This attention to place and to the living world builds coherent spaces, suited to their uses and their environment.',
    'home.statement.p2':'From private residences to the residence of the French Ambassador, I take responsibility for the full coherence of the result — complete support, from first sketch to completion and beyond.',
    'home.projects.label':'Selected work',
    'home.projects.h2':'A careful selection<br>of commissions.',
    'home.approach.label':'How I work',
    'home.approach.h2':'Listening, collaboration,<br>and the living world.',
    'home.approach.01.h4':'The clients',
    'home.approach.01.p':'A relationship of trust is essential: it allows a genuinely shared project to be built, founded on your expectations and enriched by my expertise — from design through site supervision, in constant dialogue.',
    'home.approach.02.h4':'The collaboration',
    'home.approach.02.p':'I collaborate with architects, interior designers, and urban planners to create singular outdoor spaces. Working together brings a creative and intellectual contribution that nourishes every project.',
    'home.approach.03.h4':'Plants as the project\'s material',
    'home.approach.03.p':'Plants are the raw material of a landscape project. Through their architecture, they structure space and give it meaning — a powerful lever for shaping coherent, sensitive projects.',
    'home.expertise.label':'Fields of work',
    'home.expertise.h2':'Landscape expertise,<br>from château to coast.',
    'home.expertise.01.h4':'Château parks & historic estates',
    'home.expertise.01.p':'Restoration, replanting and reinterpretation of château parks, historic estates and houses of character. Planted structures, alignments, groves and large landscape compositions are rethought through a contemporary lens, between historical continuity and transformation of the place.',
    'home.expertise.02.h4':'Private gardens',
    'home.expertise.02.p':'Villas, exceptional residences and family homes. Gardens designed from the place and its uses, where spaces, plantings and views compose a landscape to live in.',
    'home.expertise.03.h4':'Mediterranean & coastal gardens',
    'home.expertise.03.p':'A practice attentive to the specificities of Mediterranean and coastal environments. Soil, vegetation, light, climate and the relationship to the landscape found the design of the garden. Water-wise planting palettes, adapted to the conditions of the site and able to evolve.',
    'home.expertise.04.h4':'Hotels and hospitality venues',
    'home.expertise.04.p':'Landscapes designed to accompany the architecture and the different uses of the place. Gardens, patios, terraces, pools and green roofs are conceived with particular attention to circulation, views and the intimacy of spaces.',
    'home.expertise.05.h4':'Design and site supervision',
    'home.expertise.05.p':'From the first sketch through to site supervision, I handle the design of the project, the preparation of the technical files and the oversight of its execution, in dialogue with all those involved.',
    'home.expertise.06.h4':'Landscape studies',
    'home.expertise.06.p':'Landscape diagnostic, historical reading and site analysis to understand its structures, its qualities and its potential. An overall vision to guide and support the transformation of a site.',

    'home.heritage.label':'Historic estates & parks',
    'home.heritage.h2':'Your park has a history.<br>Let\'s give it a future.',
    'home.heritage.p1':'The park of a historic estate is not an ordinary garden. It is a living inheritance — original layouts, perspectives, century-old trees, forgotten walls and basins — that demands patient reading before any design.',
    'home.heritage.p2':'Climate change is now testing this heritage: box, beech, and century-old specimens are suffering across France. I design restoration and replanting plans that respect the spirit of the place while preparing it for the next fifty years.',
    'home.heritage.offer.strong':'The Park Study.',
    'home.heritage.offer.p':'An in-depth visit, a historical and landscape reading of the site, a guiding concept, and a costed roadmap — the starting point of any serious transformation.',
    'home.credibility.label':'Experience',
    'home.credibility.h2':'Trusted by discerning<br>private clients.',
    'home.credibility.p1':'From the French Ambassador\'s residence in Havana to prestige hotels delivered with Bouygues Bâtiment International — alongside private estates across France — every commission demands the same level of care, discretion, and precision.',
    'home.credibility.p2':'I collaborate regularly with architects, heritage architects, interior designers, and prestige real estate agencies who require a landscape partner capable of operating at the highest level.',
    'home.credibility.stat1.label':'Years of practice',
    'home.credibility.stat2.label':'Commissions completed',
    'home.credibility.stat3.label':'Continents',
    'home.credibility.stat4.label':'Largest estate',
    'home.cta.label':'Commissions',
    'home.cta.h2':'For private commissions<br>and collaborations.',
    'home.cta.p':'Projects are accepted selectively. If your property — château, estate, or exceptional residence — deserves a serious landscape vision, please get in touch.',

    /* Projects page */
    'projects.hero.label':'Selected work','projects.hero.h1':'Projects',
    'projects.hero.p':'A selection of projects completed in France and internationally.',
    'projects.cta.label':'Commissions',
    'projects.cta.h2':'For a commission<br>or a collaboration.',
    'projects.cta.p':'Commissions are accepted selectively. Please reach out to discuss your project.',

    /* About page */
    'about.hero.label':'The practice','about.hero.h1':'About',
    'about.hero.p':'Stéphane Magre is a landscape architect, a graduate of the École Nationale Supérieure du Paysage de Versailles.',
    'about.hero.p2':'He designs and oversees bespoke gardens and landscapes, in France and internationally — from design through to completion.',
    'about.intro.label':'Philosophy',
    'about.intro.lead':'My work rests on a close reading of the site — its topography, its geography, its living heritage. Every project is born from this attention to place and to the living world.',
    'about.intro.p1':'This approach builds coherent landscapes, suited to their uses and their environment. Composition relies on a precise planting palette, conceived as an evolving, durable material capable of answering the transformations of the climate. The form of the project thus follows from the qualities of the site, its uses and the dynamics of the living world.',
    'about.intro.p2':'The aim is to design dynamic landscapes, anchored in their context, respectful of ecological balances and open to the evolution of the living world — landscapes that carry biodiversity, where form and function align with living processes.',
    'about.philosophy.label':'Principles','about.philosophy.h2':'How I approach<br>a landscape project.',
    'about.philo.01.h4':'Reading the site',
    'about.philo.01.p':'Every site has a logic — a topography, a geography, a living heritage, an existing ecology. The project is born from observing and understanding these different layers. The form of the project follows from that understanding: look before drawing, understand before transforming.',
    'about.philo.02.h4':'Plants as raw material',
    'about.philo.02.p':'Plants are the raw material of the landscape project. Through their structure, their volumes, their textures and their seasons, they organise space, compose views and accompany uses. In dialogue with the site\'s own qualities, they make it possible to shape coherent, sensitive and living landscapes.',
    'about.philo.03.h4':'A planting palette for the century ahead',
    'about.philo.03.p':'The climate is transforming the conditions of our landscapes. I compose planting palettes able to evolve with them, reconciling resilience, permanence and the identity of the place. To preserve the spirit of a landscape is also to anticipate what it will become.',
    'about.philo.04.h4':'Landscapes for the living world',
    'about.philo.04.p':'I compose dynamic landscapes, anchored in their context, that welcome the different forms of life and take their place within the ecological balances of the site. Their permanence rests on their capacity to evolve with the living world and adapt to the transformations of their environment.',
    'about.philo.05.h4':'In dialogue with architecture',
    'about.philo.05.p':'The landscape is also built in its dialogue with architecture. I pay particular attention to proportions, materials, framing and uses, which give the project its coherence and its depth. The landscape is then neither decoration nor leftover space, but a full component of the project.',
    'about.philo.06.h4':'Design, deliver, accompany',
    'about.philo.06.p':'I accompany each project from design through to site supervision, in constant dialogue with all those involved. Once the project is complete, the relationship can continue around its maintenance, its evolution and its permanence over time.',
    'about.role.label.1':'The clients','about.role.h2.1':'A genuinely<br>shared project.',
    'about.role.p1.1':'A relationship of trust with the client is at the heart of every project. It makes it possible to listen to their expectations, to understand their uses and to bring out a project that resembles them, enriched by my eye and my expertise. The dialogue continues throughout design and construction, to build together a project that is precise, coherent and lasting. Once the garden is complete, I can also accompany its evolution over time, to preserve its quality and its spirit.',
    'about.role.p2.1':'Once the garden is complete, the relationship continues through support with maintenance and the evolution of the project, ensuring its quality and permanence over time.',
    'about.role.label.2':'The collaboration','about.role.h2.2':'Architects, interior designers<br>and urban planners.',
    'about.role.p1.2':'I work alongside architects, interior designers, urban planners, ecologists and other professionals of the project. The complementarity of perspectives and know-how nourishes the thinking and enriches the projects. Depending on the context, I work within established teams or directly with the client, bringing my eye as a landscape architect and ensuring the coherence of the whole.',
    'about.role.p2.2':'These collaborations make it possible to operate at the same level as the practices I work with — on private residences as on large international projects.',
    'about.portrait.label':'Background','about.portrait.h2':'From diploma<br>to practice.',
    'about.portrait.p1':'I hold the diploma of Paysagiste DPLG from the École Nationale Supérieure du Paysage de Versailles — with a diploma project devoted to the park of Saint-Cloud. I completed my education abroad, in contexts that substantially shaped my approach to landscape thinking.',
    'about.portrait.p2':'After some ten years devoted to large international projects — embassies, hotels, estates — I chose to dedicate my practice to private projects, to parks, historic estates and exceptional properties. A practice founded on a particular attention to place, to plants and to the long term.',
    'about.portrait.p3':'Based in Paris, I work across France and internationally.',

    /* Contact page */
    'contact.hero.label':'Get in touch','contact.hero.h1':'Contact',
    'contact.hero.p':'I work on private projects — châteaux, estates, exceptional properties — as well as in collaboration with architects and interior designers, or for study commissions.',
    'contact.hero.p2':'Every project begins with a conversation, to understand its history, its challenges and the expectations of its owners. I favour projects whose context, ambition and approach resonate with my practice and my eye as a landscape architect.',
    'contact.info.label':'A direct conversation',
    'contact.info.h2':'A first conversation.',
    'contact.info.p1':'The most direct way to reach me is by email. I reply personally to every enquiry, usually within two working days.',
    'contact.info.p2':'If you would rather speak, send me a short message by email. I will then suggest a call at a time that suits you.',
    'contact.link.email.label':'Email','contact.link.phone.label':'Phone',
    'contact.link.linkedin.label':'LinkedIn','contact.link.location.label':'Location',
    'contact.link.location.value':'Paris, France',
    'contact.note.p':'Projects are accepted selectively. I work with a small number of clients at any one time, and give each commission my full attention. If I am not available for your timeline, I will say so directly.',
    'contact.faq.label':'Information','contact.faq.h2':'Frequently asked questions.',
    'contact.faq.q1':'Where do you work?',
    'contact.faq.a1':'Based in Paris, I work across France — Île-de-France, Provence, the French Riviera, Auvergne, the Atlantic coast — as well as internationally.',
    'contact.faq.q2':'Do you work on château parks and historic estates?',
    'contact.faq.a2':'Yes. The restoration and replanting of château parks, monuments and exceptional properties is at the heart of my practice.',
    'contact.faq.q3':'What kinds of projects do you work on?',
    'contact.faq.a3':'Private estate gardens, château parks, Mediterranean and coastal gardens, hotels and hospitality venues — in France and abroad.',
    'contact.faq.q4':'Do you oversee the works?',
    'contact.faq.a4':'Yes — complete support: design, execution files and on-site supervision, through to delivery.',
    'contact.faq.q5':'How do we begin?',
    'contact.faq.a5':'With a message or a call. For parks, I offer a park study: a diagnostic, a guiding concept and a costed roadmap.',
    'contact.faq.q0':'Who is Stéphane Magre?',
    'contact.faq.a0':'Stéphane Magre is a landscape architect, a graduate of the École Nationale Supérieure du Paysage de Versailles. Based in Paris, he designs and oversees bespoke parks, gardens and landscapes — châteaux, historic estates, exceptional properties — in France and abroad.',
    'contact.faq.q6':'Can you work on an existing garden?',
    'contact.faq.a6':'Yes. Starting from a site diagnosis, I can restructure, replant or develop an existing garden, without necessarily starting over.',

    /* Shared project labels */
    'project.eyebrow':'Commission',
    'project.meta.location':'Location','project.meta.scope':'Scope',
    'project.meta.type':'Type','project.meta.status':'Status',
    'project.meta.partner':'In collaboration with',
    'project.desc.label':'Design intent',
    'project.nav.back':'All projects','project.nav.next.label':'Next project',

    /* Villa Privée */
    'villa.h1':'Villa Privée','villa.meta.location':'France',
    'villa.meta.scope':'ESQ, PRO, DCE, EXE','villa.meta.type':'Private residence, 8,000 m²',
    'villa.meta.status':'Completed',
    'villa.desc.h3':'A villa in continuity<br>with the landscape.',
    'villa.desc.p1':'From the entrance, the villa asserts itself through a contemporary architecture of clean lines and a cantilevered portico, at once surprising and weightless. The arrival garden extends this architectural gesture with an ornamental composition whose species are chosen for their decorative value, underlining the assured character of the place.',
    'villa.desc.p2':'As you move through the interior spaces, the perception reverses. On the sea side, the villa steps back to leave all the room to the site. The volumes melt into the topography and anchor themselves in the landscape. Discreet terraces slip into the natural terrain, opening framed views toward the horizon.',
    'villa.desc.p3':'Here, the garden is no longer added decoration but the very continuity of the landscape. It is imagined from native and endemic plants already present on the site: carefully collected, propagated in nurseries, then reintroduced to recreate a living, coherent ecosystem.',
    'villa.nav.prev':'All projects','villa.nav.next.title':'Ambassade de France à Cuba',

    /* Ambassade */
    'ambassade.h1':'Ambassade de France à Cuba','ambassade.meta.location':'Havana, Cuba',
    'ambassade.meta.scope':'Gardens, cultural space','ambassade.meta.type':'Diplomatic residence, 8,000 m²',
    'ambassade.meta.status':'Completed',
    'ambassade.desc.h3':'A garden of memory<br>and gathering.',
    'ambassade.desc.p1':'The garden of the residence of France was conceived as a place of sharing and of memory — a space that tells the history of the site while anchoring it in the present.',
    'ambassade.desc.p2':'The project builds on existing materials and traces: old stone, forgotten basins, and established vegetation. Great fig trees, royal palms and dense masses of exotic planting — presences respected, reworked and opened to the light to give the garden back its breath and balance.',
    'ambassade.desc.p3':'At the heart of the residence, a great lawn opens in perspective toward the statue, connecting the villa to the depth of this miniature landscape. Around this axis unfold more intimate spaces: hidden gardens, a pool, green rooms and a kitchen garden. Between heritage and vitality, the garden recovers its role: to welcome, to gather, and to reinscribe the site within a living, ecological dynamic.',
    'ambassade.nav.next.title':'Jardin Privé, Auvergne',

    /* Jardin Privé */
    'jardin.h1':'Jardin Privé','jardin.meta.location':'Auvergne, France',
    'jardin.meta.scope':'Full design &amp; execution','jardin.meta.type':'Private property, 2 ha',
    'jardin.meta.status':'Completed',
    'jardin.desc.h3':'A place both inhabited<br>and open to its environment.',
    'jardin.desc.p1':'This private garden in the Auvergne region extends over two hectares. It is structured around a large meadow, mown once a year, that opens onto a gentle valley where a stream runs. Below, a former washhouse was rediscovered, along with a tiny pool nestled among tall white willows — a discreet point of freshness prolonging the view from the terrace of the house.',
    'jardin.desc.p2':'From there, the view offers a clear reading of the valley and the surrounding hills. Around the former farmhouse, the garden unfolds in a series of living spaces: shade garden, kitchen garden, ornamental garden and replanted orchard.',
    'jardin.desc.p3':'The whole composes a place at once inhabited and open to its surroundings — an ecosystem that accompanies the natural dynamics of the site, and is deeply ecological.',
    'jardin.nav.next.title':'Hôtel Iberostar Grand Packard',

    /* Iberostar */
    'iberostar.h1':'Hôtel Iberostar Grand Packard','iberostar.meta.location':'Havana, Cuba',
    'iberostar.meta.scope':'Exterior spaces, patio, terraces, pool','iberostar.meta.type':'Luxury hotel, 3,000 m²',
    'iberostar.meta.status':'Completed',
    'iberostar.meta.partner':'Bouygues Bâtiment International',
    'iberostar.desc.h3':'Suspended gardens<br>above Havana.',
    'iberostar.desc.p1':'At the entrance to the Paseo del Prado, the Grand Packard hotel in Havana is a four-star establishment as distinguished by its elegant architecture as by the richness of its gardens. Conceived as a true green setting, it unfolds planted spaces on several levels, offering visitors a singular sensory experience at the heart of the city.',
    'iberostar.desc.p2':'On the ground floor, the interior patio draws on the great houses of Havana, with their shaded courtyards, punctuated by water and conceived as refreshing refuges from the urban heat. Directly linked to the bar and the hotel\'s festive spaces, this patio becomes a lively gathering place for concerts and events. This mineral space, clad in large slate slabs, opens onto a basin and a simple planting: Licuala grandis palms cast a light, moving shade, while below, ferns and liriopes carpet the ground in lush green.',
    'iberostar.desc.p3':'Opposite, the top-floor terraces offer an entirely different world. They unfold true hanging gardens, set on 60 cm of soil. In the upper strata, elegant Cocothrinax crinita structure the space, surrounded by an exuberant profusion of Heliconia, Alpinia, Calathea, Dieffenbachia, Monstera, Dracaena and Alocasia. Between the mineral elegance of the patio and the lushness of the terraces, the hotel offers two complementary atmospheres: one for gathering and festivity, the other intimate and contemplative, open onto horizons of green.',
    'iberostar.nav.next.title':'Les Normaliennes',

    /* Les Normaliennes */
    'normaliennes.h1':'Les Normaliennes','normaliennes.meta.location':'Auvergne, France',
    'normaliennes.meta.scope':'Lead consultant — APD, PRO, DCE, EXE','normaliennes.meta.type':'Urban garden, 2 ha',
    'normaliennes.meta.status':'Completed',
    'normaliennes.meta.partner':'Cap Paysages',
    'normaliennes.desc.h3':'A traversing garden<br>at the heart of the block.',
    'normaliennes.desc.p1':'The requalification of the former IUFM grounds restores genuine landscape coherence and renewed value to this urban block. At the heart of a mix of school and housing, the site undergoes a major transformation: creating a traversing pedestrian walk while installing a generous garden at the centre of the block.',
    'normaliennes.desc.p2':'The traversing walk, laid in Volvic stone setts and declined in paving, mineral stabilisation, benches and large natural-form slabs, structures the site and gives it a shared identity.',
    'normaliennes.desc.p3':'On either side unfolds a linear garden of nearly two hectares, composed mainly of grasses joined by perennials, euphorbias, petasites and sedums, punctuated by structural plantings of magnolias and serviceberries. The whole forms a garden at once free in its planting and rigorously drawn, offering landscape continuity from façade to façade.',
    'normaliennes.nav.next.title':'Hôtel Melina International',

    /* Hôtel Melina International */
    'melina.h1':'Hôtel Melina International','melina.meta.location':'Varadero, Cuba',
    'melina.meta.scope':'ESQ, APD, PRO, DCE, EXE','melina.meta.type':'Luxury hotel, 6 ha',
    'melina.meta.status':'Completed',
    'melina.meta.partner':'Bouygues Bâtiment International',
    'melina.desc.h3':'A botanical garden<br>facing the sea.',
    'melina.desc.p1':'The gardens of the International hotel span six hectares, wrapping an imposing building in an abundance of vegetation. Conceived as a true botanical garden, they offer an immersion in tropical diversity, revealing the identity of the site through the richness of its species. Organic paths and sunken walkways invite a stroll through stratified vegetation, where ground covers, shrubs, palms and large trees interweave.',
    'melina.desc.p2':'A large part of the garden unfolds between the building and the sea, where the pool dissolves into this lush tropical landscape. Here you find emblematic species — heliconias, dracaenas, alocasias, alpinias, monsteras, aralias, sugar cane, cycas revoluta and frangipanis — complemented by liriopes and spathiphyllums as ground cover.',
    'melina.desc.p3':'Royal palms, Washingtonia robusta, coconut palms, Beaucarnea recurvata and Santa Lucia palms mingle with Schefflera, Cordia sebestena and Plumeria, composing a dense canopy and a profusion of tropical planting by the sea.',
    'melina.nav.next.title':'Hôtel Prado Malecón',

    /* Hôtel Prado Malecón */
    'prado.h1':'Hôtel Prado Malecón','prado.meta.location':'Havana, Cuba',
    'prado.meta.scope':'APD, PRO, DCE, EXE','prado.meta.type':'Luxury hotel, 1,000 m²',
    'prado.meta.status':'Completed',
    'prado.meta.partner':'Bouygues Bâtiment International',
    'prado.desc.h3':'A living heart<br>between Prado and Malecón.',
    'prado.desc.p1':'At the corner of the Paseo del Prado and the Malecón, the Prado hotel is organised around a 300 m² patio — a true living heart connecting the building\'s two wings. A central path of white terrazzo crosses this 25-metre-long space, inviting the visitor to wander through lush tropical vegetation.',
    'prado.desc.p2':'Planted in open ground, the garden holds Dracaena, Heliconia, ferns, Anthurium, Calathea and Begonia, along with climbers such as Monstera deliciosa that will in time clothe the stainless-steel cables stretched overhead. Carefully placed benches offer moments of pause in this intimate, traversing space.',
    'prado.desc.p3':'The terraces, exposed to the sea wind, are punctuated with pots of Areca palms. From the lower terraces, the sea and the Malecón promenade stretch out before you; from the upper terraces, the gaze embraces the seafront and the Paseo del Prado, leading toward the historic centre, the Grand Theatre and the Capitol.',
    'prado.nav.next.title':'Pôle Touristique Cayo Cruz',

    /* Pôle Touristique Cayo Cruz */
    'cayo.h1':'Pôle Touristique Cayo Cruz','cayo.meta.location':'Cayo Cruz, Cuba',
    'cayo.meta.scope':'APD, PRO, DCE, EXE','cayo.meta.type':'Resort, 15 ha',
    'cayo.meta.status':'Completed',
    'cayo.meta.partner':'Bouygues Bâtiment International',
    'cayo.desc.h3':'Preserving an island,<br>inhabiting its landscape.',
    'cayo.desc.p1':'The Coco de Mar hotel sits on a 15-hectare plot, on a small Caribbean island. The project is organised around buildings with circular patios and organic forms, set within the existing tropical vegetation.',
    'cayo.desc.p2':'The central challenge is to preserve the natural landscape: paths and sand tracks are redrawn on site to skirt the remarkable trees and specimens, while respecting the subtle topography of the place, marked by fossilised sands and a specific flora.',
    'cayo.desc.p3':'At the heart of this setting, a leisure garden holds a pool whose organically shaped decks settle into the site\'s topography. In the patios, an ornamental planting palette dialogues with native and endemic species, striking a balance between new species and the enhancement of the existing plant heritage.',
    'cayo.nav.next.title':'Hôtel Corona',

    /* Hôtel Corona */
    'corona.h1':'Hôtel Corona','corona.meta.location':'Havana, Cuba',
    'corona.meta.scope':'APD, PRO, DCE, EXE','corona.meta.type':'Luxury hotel, 1,000 m² patio',
    'corona.meta.status':'Completed',
    'corona.meta.partner':'Bouygues Bâtiment International',
    'corona.desc.h3':'A tropical refuge<br>in the centre of the city.',
    'corona.desc.p1':'At the heart of this former building, converted into a five-storey hotel, opens a central patio of nearly 1,000 m² — a mineral and living setting, at once a place of rest and of sensory immersion. The black granite floor catches the light and deepens the hushed atmosphere of the place, between coolness and depth.',
    'corona.desc.p2':'Around this space flourishes a lush vegetation. To reach the central terrace, visitors cross this two-to-five-metre depth of planting beneath a canopy of exotic foliage, before emerging at the heart of the patio — a well of light open to the sky, where tables are set around the bar.',
    'corona.desc.p3':'The plantings compose a rich tropical palette — Anthurium, Begonia, Calathea and Heliconia in the shaded understory, Dracaena and Monstera deliciosa as a dense framework, Licuala grandis, Washingtonia and the majestic Ravenala madagascariensis, travellers\' trees, raising their silhouettes above the patio. A tropical refuge in the centre of the city.',
    'corona.nav.next.title':'La Traversée',

    /* La Traversée */
    'traversee.h1':'La Traversée','traversee.meta.location':'Atlantic coast, France',
    'traversee.meta.scope':'APD, PRO, DCE, EXE','traversee.meta.type':'Coastal promenade, 800 ml',
    'traversee.meta.status':'Completed',
    'traversee.desc.h3':'An experience of crossing<br>between land and sea.',
    'traversee.desc.p1':'This promenade is set within a natural landscape — a site of rock, water and halophytic coastal vegetation.',
    'traversee.desc.p2':'Between the sea and the village, a space of transition opens, crossed by a river that mingles with the tidal flows. It is here that the path finds its place, not as a rupture, but as a device of continuity between two parts of the site.',
    'traversee.desc.p3':'Rather than a mere link, the promenade becomes an experience of crossing.',
    'traversee.nav.next.title':'Villa Privée',

    /* Project card descriptions */
    'card.villa.desc':'A garden of native planting, continuing both the landscape and the architecture.',
    'card.ambassade.desc':'The garden of the French Ambassador\'s residence in Havana, a place of gathering and memory.',
    'card.jardin.desc':'A two-hectare property in Auvergne: meadow, washhouse, orchard and kitchen garden.',
    'card.iberostar.desc':'A ground-floor patio and hanging rooftop gardens, in the heart of Havana.',
    'card.normaliennes.desc':'The requalification of an urban block in Auvergne: a Volvic stone walk and two hectares of gardens.',
    'card.melina.desc':'Six hectares of tropical botanical garden by the sea, in Varadero.',
    'card.prado.desc':'A 300 m² garden patio between the Paseo del Prado and the Malecón.',
    'card.cayo.desc':'Fifteen hectares on a Caribbean island, designed to preserve the existing landscape.',
    'card.corona.desc':'A 1,000 m² patio inside a converted historic building: black granite and tropical planting.',
    'card.traversee.desc':'A coastal promenade between rock, water and salt-tolerant planting.',
  }
};

/* ── Engine ─────────────────────────────────────────────── */
let currentLang = localStorage.getItem('sm_lang') || 'fr';

function t(key) {
  return (T[currentLang] && T[currentLang][key]) || (T['en'][key]) || key;
}

function applyTranslations() {
  document.querySelectorAll('[data-i18n]').forEach(el => {
    el.textContent = t(el.dataset.i18n);
  });
  document.querySelectorAll('[data-i18n-html]').forEach(el => {
    el.innerHTML = t(el.dataset.i18nHtml);
  });
  document.querySelectorAll('[data-i18n-aria]').forEach(el => {
    el.setAttribute('aria-label', t(el.dataset.i18nAria));
  });
  /* lang toggle button */
  document.querySelectorAll('.lang-btn').forEach(btn => {
    const isActive = btn.dataset.lang === currentLang;
    btn.classList.toggle('lang-btn--active', isActive);
  });
  document.documentElement.lang = currentLang;
}

function setLanguage(lang) {
  if (!T[lang]) return;
  currentLang = lang;
  localStorage.setItem('sm_lang', lang);
  applyTranslations();
}

document.addEventListener('DOMContentLoaded', () => {
  applyTranslations();
  document.querySelectorAll('.lang-btn').forEach(btn => {
    btn.addEventListener('click', () => setLanguage(btn.dataset.lang));
  });
});
